...
// fetch markdown
{
  ...
  const main_page_markdown = await essentials.fetchMarkdownFile(`${route.params.franchise}/index.md`, true);

  if (main_page_markdown) {
  ...
  <div :class="!mounted ? `overflow-hidden` : ``">
    <Wiki_contents :franchise="route.params.franchise as string" :additional_data="history_data">
      <div class="wiki_header justify-between">
        ...
        <Wiki_header :franchise="route.params.franchise" :franchise_image="the_franchise_data.wiki_head_image" :raw_markdown="`${essentials.getOriginURL()}/api/msc/files/${route.params.franchise
          }/index.md`" :page_count="the_franchise_data.page_count" :saihex_creation="the_franchise_data.saihex_creation" />
        <!-- logo -->
