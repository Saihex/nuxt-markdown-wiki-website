...
// initial search
{
  ...
  {
    const url = new URL(`${getOriginURL()}/api/msc/dirSearch`);

    // Sanitize and encode the query value
    url.searchParams.set("directoryMode", "true");
    url.searchParams.set("searchEntry", inputValue.value || "");

    FetchURL = url.toString();
  }

  fetchStatus.value = 1;
  const FetchResult = await useFetch(FetchURL);

  if (FetchResult.error.value) {
    fetchStatus.value = 3;
  } else {
    const data = FetchResult.data.value as string[];

    for (const key in data) {
      const _franchise_data = await useFetch(
        `${getOriginURL()}/api/msc/frontmatter/${data[key]}/index.md`
      );

      if (!_franchise_data.error.value && _franchise_data.data.value) {
        const frontmatter = _franchise_data.data.value as franchise_data;
        frontmatter.dynamic_path = data[key];
        results.value[data[key]] = frontmatter;
      }
    }
...
async function searchLogic(searchEntry: string) {
  ...
  fetchStatus.value = 1;
  const FetchResult: string[] = await $fetch(FetchURL);

  if (FetchResult.length > 0) {
    for (const key in FetchResult) {
      const _franchise_data: franchise_data = await $fetch(
        `${getOriginURL()}/api/msc/frontmatter/${FetchResult[key]}/index.md`
      );
    ...
    <div>
        <div class="min-h-svh">
            ...
            <div class="result_box">
                <NuxtLink v-for="one_of_rsult in results" :class="
            one_of_rsult.saihex_creation
              ? `result_boxes`
